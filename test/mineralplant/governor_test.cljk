...
(defn- fresh-store []
  (let [st (store/mem-store)]
    ...
    (store/register-plant! st {:plant-id "P-1" :name "Kobo Cement Plant" :max-supply-cost 2000})
    st))
...
(deftest always-escalates-safety-concern-even-at-high-confidence
  (testing "a heavy-machinery crush/entanglement, heat-exposure or dust-exposure concern always requires human sign-off"
    (let [st (fresh-store)
...
(deftest default-mock-advisor-proposals-never-self-trip-on-scope-exclusion
  (testing "the governor's scope-exclusion term list must never match the mock advisor's own default rationale text for any allowlisted op — CLAUDE.md's known self-tripping bug pattern (rationale legitimately contains bare nouns like 'cement'/'kiln'/'dust'/'plant'/'safety officer', but never the full finalization-action phrases)"
    (let [st (fresh-store)
      ...
      (doseq [o ops]
        (let [request {:operator-id "operator-1" :op o :plant-id "P-1"
                        :stake :low :task "routine cement production-run task"
