(ns mineralplant.actor
  "MineralPlantCoordActor — the ISCO-08 8114 cement, stone and other
  mineral products machine operator plant scheduling/logistics
  coordination actor as a `langgraph.graph/state-graph`
  (ADR-2607121000 / CLAUDE.md Actors section). One graph run = one
  plant-coordination operation request (intake -> advise -> govern ->
  ...
  The unconditional invariant: the Mineral Products Plant Coordination
  Advisor can never directly commit an operating record the
  MineralPlantCoordGovernor refuses, and can never make this actor
  finalize a machine-operation-execution decision (e.g. deciding to
  run or proceed with a specific forming/pressing/kiln machine cycle)
  or a plant-safety-clearance decision (e.g. declaring the plant floor
  safety-cleared), or override a plant safety officer's judgment —
  every commit-record! call is gated behind `:decide`, and that
  decision class is a permanent hard block regardless of disposition
  path. This actor coordinates PLANT SCHEDULING/LOGISTICS ONLY — it
  never operates plant machinery or makes safety-clearance decisions
  itself."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [mineralplant.advisor :as advisor]
            [mineralplant.governor :as governor]
            [mineralplant.store :as store]))

(defn build-graph
  "Build a compiled MineralPlantCoordActor graph. `store` implements
  `mineralplant.store/Store`. `advisor` implements
  `mineralplant.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:operator-id (:operator-id request)
                                    :op (:op proposal)
                                    :plant-id (:plant-id proposal)
                                    :payload proposal}]
