(ns mineralplant.advisor
  "Mineral Products Plant Coordination Advisor — proposing a cement/
  stone/mineral-products plant scheduling/logistics coordination
  operation (log a work record, schedule a crew operation, flag a
  safety concern, coordinate a raw-material/spare-parts supply order)
  from a crew roster, plant registration and safety-reporting policy.
  Swappable mock/llm; the advisor ONLY proposes —
  `mineralplant.governor` independently gates every proposal and
  always escalates safety concerns and above-threshold supply orders.
  The advisor never proposes to directly finalize a
  machine-operation-execution decision (e.g. deciding to run or
  proceed with a specific forming/pressing/kiln machine cycle) or a
  ...
  scope. Modeled closely on cloud-itonami-isco-7211's
  foundrycoord.advisor for the industrial-plant-safety-domain shape.
...
(def ^:private system-prompt
  "You are a cement, stone and mineral products machine-operator
   ...
   plant machinery or makes safety-clearance decisions itself. Safety
