(ns mineralplant.store
  "SSoT for the ISCO-08 8114 cement, stone and other mineral products
  machine operator plant scheduling/logistics coordination actor
  ...
  inventory/progress record logging and raw-material/spare-parts
  supply-order coordination for a cement, stone and mineral products
  machine-operator crew under this advisor/governor pair, which never
  dispatches hardware itself, never operates plant machinery itself,
  ...
  exclusive judgment). Modeled closely on cloud-itonami-isco-7211's
  foundrycoord.store for the industrial-plant-safety-domain shape.

  Domain:

    operator — a registered cement/stone/mineral-products machine
               operator crew member (:operator-id, :name)
    plant    — a registered cement/stone/mineral-products plant site
               {:plant-id :name :max-supply-cost}. `:max-supply-cost`
    ...
    record   — a committed operating record (a logged production-run/
               ...
               raw-material/spare-parts supply order) — written ONLY
               via commit-record!.
