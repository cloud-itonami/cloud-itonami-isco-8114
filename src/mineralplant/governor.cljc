(ns mineralplant.governor
  ...
  for a cement, stone and mineral products machine-operator crew. The
  governor never dispatches hardware itself, never operates plant
  ...
  (e.g. declaring the plant floor safety-cleared), and never overrides
  ...
  judgment (README's 'Robotics premise': this actor coordinates PLANT
  SCHEDULING/LOGISTICS ONLY — it never operates machinery or makes
  ...
  HARD invariants (:hard? true, ALWAYS :hold, never overridable):
    ...
    3. no-actuation           — proposal :effect must be :propose (the
                                ...
                                never operates plant machinery itself;
    ...
    5. scope-excluded action  — any proposal to directly finalize a
                                ...
                                specific forming/pressing/kiln machine
                                cycle) or a plant-safety-clearance
                                decision (e.g. declaring the plant
                                floor safety-cleared), or to override a
                                ...
                                \"proceed with the machine operation\" /
                                \"authorize the machine operation\" /
                                ...
                                crew operation for production-run
  ...
  regardless of confidence):
    6. :op :flag-safety-concern (a heavy-machinery crush/entanglement,
                                heat-exposure or dust-exposure concern
...
(def ^:private scope-excluded-ops
  #{:finalize-machine-operation-decision :authorize-machine-operation
    :proceed-with-machine-operation :run-machine-operation-cycle
...
;; ("cement", "stone", "mineral", "kiln", "dust", "safety", "plant",
...
(def ^:private scope-excluded-phrases
  ["proceed with the machine operation" "proceed with machine operation"
   "authorize the machine operation" "authorize machine operation"
   "finalize the machine-operation decision" "finalize the machine operation decision"
  ...
  (let [{:keys [op rationale]} proposal]
    (cond-> []
      ...
      (conj {:rule :no-actuation
             :detail "effect は :propose のみ許可（governor は plant machinery を直接操作しない）"})
      ...
      (conj {:rule :scope-excluded-action
             :detail "machine-operation 実行判断の確定・plant safety clearance 判断の確定・plant safety officer の判断の上書きは、この actor の権限外 — 常に永続ブロック"}))))
